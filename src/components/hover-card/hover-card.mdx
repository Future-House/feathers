...
<div key={sub.name}>
<h3 className="sb-unstyled text-xl text-foreground font-bold mt-6 mb-3">{sub.name}</h3>
{sub.description && <p className="sb-unstyled text-base text-muted-foreground mb-4">{sub.description}</p>}
<PropsTable
