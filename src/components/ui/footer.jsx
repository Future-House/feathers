...
const Logo = forwardRef((props, ref) => (
    <a 
        href="https://futurehouse.org" 
        target="_blank" 
        rel="noopener noreferrer" 
        aria-label="FutureHouse Home"
        ref={ref}
    >
        <svg
            id="FutureHouse_Logo"
...
const Footer = forwardRef(({ footerProps, links = [], disclaimers = [], ...rest }, ref) => {
    const linksToRender = [...links, ...internalLinks]
    const { theme } = useTheme()

    return (
        <Box ref={ref} className="bg-background" {...rest}>
            <Box asChild className="w-full" {...footerProps}>
                <footer role="contentinfo">
                    <Stack className="justify-between items-start py-3 md:py-4 space-y-8 lg:flex-row">
                        <Stack className="space-y-6 md:space-y-8 items-start">
                            <Logo fill={theme === 'dark' ? 'white' : 'black'} />
                            {disclaimers.length > 0 && (
                                disclaimers.map((disclaimer, idx) => (
                                    <Fragment key={idx}>{disclaimer}</Fragment>
                                ))
                            )}
                        </Stack>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 w-full lg:w-auto">
                            {linksToRender.map((group, idx) => (
                                <Stack key={idx} className="space-y-4 lg:min-w-40">
                                    ...
                                    <Stack className="space-y-3">
                                        {group.links.map((link, idx) => (
                                            <Button
                                                ...
                                                asChild
                                                variant="ghost"
                                                className="p-0 h-auto font-normal hover:font-bold focus:font-bold"
                                            >
                                                <a 
                    ...
                    <Stack className="pt-8 pb-12 justify-between md:flex-row items-center space-y-4 md:space-y-0">
                        ...
                        <div className="flex space-x-4">
                            ...
                            >
                                <a 
                                    href="https://www.linkedin.com/company/futurehouse?trk=public_post_feed-actor-name" 
                                    target="_blank" 
                                    rel="noopener noreferrer"
                                >
                            ...
                            >
                                <a 
                                    href="https://x.com/futurehousesf" 
                                    target="_blank" 
                                    rel="noopener noreferrer"
                                >
